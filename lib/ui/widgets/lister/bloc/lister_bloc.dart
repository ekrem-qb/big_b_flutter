...
abstract class ListerBloc<T>
      ...
      db
          ...
          .onPostgresChanges(
            ...
            callback:
                (final payload) => add(
                  joinTables.isEmpty
                      ? _ListerEventDataUpdated(payload: payload)
                      : const ListerEventLoadRequested(),
                ),
          )
          .subscribe(),
      ...joinTables.map((final joinTable) {
        return db
            .channel(generateHash())
            .onPostgresChanges(
              ...
              callback:
                  (final payload) => add(const ListerEventLoadRequested()),
            )
            .subscribe();
      }),
    ];
  ...
  Future<void> _onLoadRequested(
    ...
    try {
      ...
      final count =
          await (filters != null ? filters!.$1(countQuery) : countQuery);
      final endIndex = max(itemsPerLoad, switch (state) {
        StatusOfData(:final data) => data.items.length,
        _ => 0,
      });
      final query = db.from(tableName).select(fieldNames);
      final items =
          await (filters != null ? filters!.$1(query) : query)
              .limit(endIndex)
  ...
  Future<void> _onLoadAfterRequested(
    ...
    try {
      switch (state) {
        case StatusOfData(:final data, :final copyWith):
          if (event.index < data.items.length) return;
          if (data.items.length >= data.totalCount) return;

          final endIndex = min(
            (data.items.length - 1) + itemsPerLoad,
            data.totalCount - 1,
          );
          final query = db.from(tableName).select(fieldNames);
          final items =
              await (filters != null ? filters!.$1(query) : query)
                  .range(data.items.length, endIndex)
                  .order(orderBy, ascending: ascending)
                  .withConverter(converter) ??
              const [];

          emit(copyWith(data: data.copyWith(items: [...data.items, ...items])));
        default:
          return;
      }
    } catch (e) {
  ...
  StatusOfData<ListerState<T>, String>? _insert(
    ...
    final newItems =
        filters != null && !filters!.$2(newItem)
            ? null
            : _insertIntoList(newItem, currentState.data.items);
  ...
  StatusOfData<ListerState<T>, String>? _update(
    ...
    if (filters != null) {
      ...
      if (filters!.$2(newItem)) {
        final itemsWithoutOldRecord = _deleteFromList(
          payload.oldRecord,
          currentState.data.items,
        );
        newItems = _insertIntoList(
          newItem,
          itemsWithoutOldRecord ?? currentState.data.items,
        );
    ...
    return newItems != null
        ? StatusOfData(
          newCount != null
              ...
              : currentState.data.copyWith(items: newItems),
        )
        : null;
  }
  ...
  List<T>? _insertIntoList(final T newItem, final List<T>? items) {
    ...
    for (var i = 0; i < items.length; i++) {
      if (isAfter(newItem, items[i])) {
        return [...items.sublist(0, i), newItem, ...items.sublist(i)];
      }
    }

    return _isFullyLoaded() ? [...items, newItem] : null;
  }
