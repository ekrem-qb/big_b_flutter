...
sealed class PlayerEvent with _$PlayerEvent {
  ...
  const factory PlayerEvent.audioLoadRequested() =
      PlayerEventAudioLoadRequested;
  ...
  const factory PlayerEvent.playingChanged({required final bool isPlaying}) =
      _PlayerEventPlayingChanged;

  const factory PlayerEvent.error({required final String error}) =
      _PlayerEventError;
  ...
  const factory PlayerEvent.seekRequested(final Duration position) =
      PlayerEventSeekRequested;

  const factory PlayerEvent.playPauseButtonPressed() =
      PlayerEventPlayPauseButtonPressed;
}
